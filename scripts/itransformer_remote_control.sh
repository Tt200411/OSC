...
HOSTS_DEFAULT="10.21.53.82 10.21.53.113 10.21.53.142 10.21.53.162"
DISABLED_HOSTS="10.20.12.248 10.20.12.247 10.21.53.62"
REMOTE_USER="${REMOTE_USER:-testsv}"
REMOTE_DIR="${REMOTE_DIR:-~/project/osc_informer}"
HOSTS="${HOSTS:-${HOSTS_DEFAULT}}"
RUN_TS="${RUN_TS:-$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-.aris/itransformer_remote_${RUN_TS}}"
...
inventory() {
  local path="${OUT_DIR}/itransformer_gpu_inventory_${RUN_TS}.md"
  {
    echo "# iTransformer GPU Inventory"
    echo
    echo "Generated: $(date '+%Y-%m-%dT%H:%M:%S%z')"
    echo
    echo "Disabled hosts: ${DISABLED_HOSTS}"
    echo
    echo "| host | status | gpu | memory.used | memory.total | itransformer | data_2_2 | venv | notes |"
    echo "| --- | --- | --- | --- | --- | --- | --- | --- | --- |"
  } > "${path}"
  for host in ${HOSTS}; do
    case " ${DISABLED_HOSTS} " in *" ${host} "*) continue ;; esac
    set +e
    output="$(ssh_cmd "${host}" "cd ${REMOTE_DIR} && gpu=\$(nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv,noheader,nounits 2>/dev/null | head -1 || true); ok=0; for f in lee_ocil/ETT-small/ETTh2.csv lee_ocil/ETT-small/ETTm2.csv; do [ -f \"\$f\" ] && ok=\$((ok+1)); done; [ -f iTransformer/run.py ] && it=yes || it=no; [ -x lee_ocil/.venv/bin/python ] && venv=yes || venv=no; printf '%s\n%s\n%s\n%s\n' \"\$gpu\" \"\$it\" \"\$ok\" \"\$venv\"" 2>&1)"
    rc=$?
    set -e
    if [[ "${rc}" -ne 0 ]]; then
      note="${output//$'\n'/ }"
      echo "| ${host} | ssh_failed |  |  |  | no | 0/2 | no | ${note} |" >> "${path}"
      continue
    fi
    output="$(printf '%s\n' "${output}" | tr -d '\r' | sed '/^[[:space:]]*$/d')"
    gpu_line="$(printf '%s\n' "${output}" | sed -n '1p')"
    itransformer="$(printf '%s\n' "${output}" | sed -n '2p')"
    data_count="$(printf '%s\n' "${output}" | sed -n '3p')"
    venv="$(printf '%s\n' "${output}" | sed -n '4p')"
    IFS=',' read -r gpu_name mem_used mem_total <<< "${gpu_line}"
    echo "| ${host} | ok | ${gpu_name} | ${mem_used} MiB | ${mem_total} MiB | ${itransformer} | ${data_count}/2 | ${venv} |  |" >> "${path}"
  done
  echo "${path}"
}
...
sync_all() {
  for host in ${HOSTS}; do
    case " ${DISABLED_HOSTS} " in *" ${host} "*) continue ;; esac
    ITRANSFORMER_SSH_PASSWORD="${PASSWORD_ENV:-}" scripts/sync_itransformer_remote.sh "${REMOTE_USER}@${host}" "${REMOTE_DIR}"
  done
}
...
launch_queues() {
  ...
  for host in ${HOSTS}; do
    case " ${DISABLED_HOSTS} " in *" ${host} "*) continue ;; esac
    local manifest="${manifest_dir}/itransformer_queue_manifest_${host//./_}.json"
    [[ -f "${manifest}" ]] || continue
    local remote_rel=".aris_queue/itransformer/${RUN_TS}_${host//./_}"
    ssh_cmd "${host}" "mkdir -p \"\$HOME/${remote_rel}/logs\" \"\$HOME/.aris_queue\""
    scp_cmd "${QUEUE_MANAGER}" "${host}" ".aris_queue/itransformer_queue_manager.py"
    scp_cmd "${manifest}" "${host}" "${remote_rel}/manifest.json"
    local screen_name="ITQM_${RUN_TS}_${host//./_}"
    ssh_cmd "${host}" "screen -S ${screen_name} -X quit >/dev/null 2>&1 || true; screen -dmS ${screen_name} bash -lc 'cd ${REMOTE_DIR}/iTransformer && /home/testsv/project/osc_informer/lee_ocil/.venv/bin/python \"\$HOME/.aris_queue/itransformer_queue_manager.py\" --manifest \"\$HOME/${remote_rel}/manifest.json\" --state \"\$HOME/${remote_rel}/queue_state.json\" --log-dir \"\$HOME/${remote_rel}/logs\" > \"\$HOME/${remote_rel}/queue_mgr.log\" 2>&1'"
    echo "${host} ${remote_rel}" >> "${OUT_DIR}/itransformer_launched_queues_${RUN_TS}.txt"
  done
}
...
print('counts:', counts)
for job in [j for j in jobs if j.get('status') == 'running'][:8]:
...
if os.path.isdir(log_dir):
    for name in sorted(os.listdir(log_dir))[-80:]:
        if name.endswith('.log'):
            text = open(os.path.join(log_dir, name), errors='replace').read()[-30000:]
            if pattern.search(text):
                hits.append(name)
print('log_alerts:', ','.join(hits[:12]) if hits else 'none')
PY
...
fetch_queues() {
  ...
  while read -r host remote_rel; do
    ...
    if [[ -n "${result_match}" ]]; then
      rsync_pull_cmd "${host}" "${REMOTE_DIR}/iTransformer/results/" "${host_dir}/results/" \
        --prune-empty-dirs --include '*/' --include 'summary.csv' \
        --include "*${result_match}*/config.json" --include "*${result_match}*/metrics.npy" \
        --include "*${result_match}*/pred.npy" --include "*${result_match}*/true.npy" --exclude '*' || true
    else
      rsync_pull_cmd "${host}" "${REMOTE_DIR}/iTransformer/results/" "${host_dir}/results/" \
        --prune-empty-dirs --include '*/' --include 'summary.csv' --include 'config.json' \
        --include 'metrics.npy' --include 'pred.npy' --include 'true.npy' --exclude '*' || true
    fi
  done < "${launched_file}"
}
